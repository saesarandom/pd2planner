...
### Failed Complete Refactor (PRs #311-315):
Attempted complete overhaul with unified itemState management system.

Issues encountered:
- All items sharing same corruption (not per-item persistence)
- itemState lagging behind when switching items
- Too many interconnected changes needed
- Broke existing state persistence system
...
## Next Steps:
Need a simpler, incremental approach that:
1. Preserves per-item corruption state (existing system already has this)
