class DPSSimulator {
  ...
  calculateDPS() {
    const damageText =
      document.getElementById("damageOutput")?.textContent || "0-0";
    const { min: minDmg, max: maxDmg } = this.parseDamageRange(damageText);
    const critChance =
      parseFloat(document.getElementById("criticalhitcontainer")?.value || 0) /
      100;
    const critMultiplier = parseFloat(
      document.getElementById("crithitmultipliercontainer")?.textContent || 1
    );
